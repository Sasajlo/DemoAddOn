...
export default class Info {
    show(player) {
        ...
        form.label(`§a§lThank you for playing this demo!§r\n\n` +
        ...
        form.label("Settings:");
        form.toggle("Fog", { defaultValue: player.getDynamicProperty("demo_fog") });
        form.slider("Hook Speed", 1, 10, { defaultValue: player.getDynamicProperty("demo_hook_speed") });
        ...
        form.show(player).then((formData) => {
            if (!formData.formValues)
                return;
            // Toggle fog
            const fogValue = formData.formValues[2];
            if (fogValue) {
            ...
            // Save hook speed
            player.setDynamicProperty("demo_hook_speed", formData.formValues[3]);
            // Play sound
