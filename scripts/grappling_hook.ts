...
export function onItemUse(event: ItemUseAfterEvent) {
  ...
  // Check cooldown
  if (player.getItemCooldown("demo_grappling_hook") > 0) return;
  ...
  leashableComponent?.leashTo(player);
...
function spawnMount(player: Player, hook: Entity) {
  ...
  leashableComponent?.leashTo(mount);

  const targetDirection = Vector3Utils.normalize(Vector3Utils.subtract(hook.location, player.location));

  const speedModifier =
    (((player.getDynamicProperty("demo_hook_speed") as number) - 1) / 9) * (MOUNT_SPEED_MAX - MOUNT_SPEED_MIN) +
    MOUNT_SPEED_MIN;
  ...
  const runHandle = system.runInterval(() => {
    ...
    if (
      rideableComponent?.getRiders().length === 0 ||
...
function disconnect(player: Player, hook: Entity, mount: Entity) {
  hook.remove();
  mount.remove();
